...
func TestSaveDiaryEntry(t *testing.T) {
	for idx, data := range []struct {
		...
		{ //05// invalid id param
			...
			method:    http.MethodPost,
			params:    httprouter.Params{{Key: "date", Value: "2022-03-12"}},
			in:        url.Values{"id": {"hi"}, "amount": {"120"}, "time": {mock.Date1Time}, "recipe": {""}},
			setCookie: true,
			entries:   []core.DiaryEntry{},
			status:    http.StatusNoContent,
		},
		{ //06// invalid amount param
			...
			method:    http.MethodPost,
			params:    httprouter.Params{{Key: "date", Value: "2022-03-12"}},
			in:        url.Values{"id": {"1"}, "amount": {"lots"}, "time": {mock.Date1Time}, "recipe": {""}},
			setCookie: true,
			entries:   []core.DiaryEntry{},
			status:    http.StatusNoContent,
		},
		{ //07// invalid time param
			...
			method:    http.MethodPost,
			params:    httprouter.Params{{Key: "date", Value: "2022-03-12"}},
			in:        url.Values{"id": {"1"}, "amount": {"120"}, "time": {"now"}, "recipe": {""}},
			setCookie: true,
			entries:   []core.DiaryEntry{},
			status:    http.StatusNoContent,
		},
		{ //08// connection failure
			...
			in:        url.Values{"id": {"1"}, "amount": {"120"}, "time": {mock.Date1Time}, "recipe": {""}},
			setCookie: true,
			entries:   []core.DiaryEntry{},
		...
		{ //11// replace entries
			db: mock.NewDB().WithEntries(
				...
				core.DiaryEntry{Date: mock.Date1, Food: core.Ingredient{ID: 2, Amount: 50}, Recipe: "Rec1"},
				core.DiaryEntry{Date: mock.Date2, Food: core.Ingredient{ID: 1, Amount: 30}},
			),
			method: http.MethodPut,
			params: httprouter.Params{{Key: "date", Value: mock.Date1Date}},
			in: url.Values{
			...
			entries: []core.DiaryEntry{
				{Date: mock.Date1, Food: core.Ingredient{ID: 1, Amount: 120}},
				{Date: mock.Date1, Food: core.Ingredient{ID: 2, Amount: 210}, Recipe: "Rec1"},
				{Date: mock.Date2, Food: core.Ingredient{ID: 1, Amount: 30}},
			},
			status: http.StatusNoContent,
		},
	} {
...
func TestDiary(t *testing.T) {
	for idx, data := range []struct {
		...
		{ //08// success for day
			db:        mock.NewDB().WithDays(mock.Diary210101(), mock.Diary210102(), mock.Diary220312()),
			params:    httprouter.Params{{Key: "year", Value: "2022"}, {Key: "month", Value: "3"}, {Key: "day", Value: "12"}},
			setCookie: true,
			out:       fmt.Sprintf(`[%s]`, mock.Diary220312Json),
			status:    http.StatusOK,
		},
	} {
